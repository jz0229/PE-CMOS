%For plotting pCMOS data in Fig.2C

%load the data
load('./raw_data/fig2_C_vid.mat');
load('./raw_data/fig2_C_mask.mat');

%%
%draw an ROI
figure(1)
imagesc(sum(pcmos_vid,3)/length(pcmos_vid), [50 300]); colormap(gray); 
...
figure(2); 
a = subplot(1,2,1); 
imagesc(sum_img(10:end,1:50), [130 290]); colormap(gray); 
title('Max. Project of the pCMOS video'); 
b = subplot(1,2,2); 
...
%get a segement of the data
Ph0_sec = Ph0_filt(xlow:xhigh);
Ph1_sec = Ph1_filt(xlow:xhigh);
Ph2_sec = Ph2_filt(xlow:xhigh);
Ph3_sec = Ph3_filt(xlow:xhigh);

%select a noise range
noise_std_range = [1000:1450];
...
hold on; 
stem(x_range(1)*1/200, 5*std(Ph0_noise(noise_std_range))); hold off; 
ylim([ylo yhi]);
...
hold on; 
stem(x_range(1)*1/200, 5*std(Ph1_noise(noise_std_range))); hold off; 
ylim([ylo yhi]);
...
hold on; 
stem(x_range(1)*1/200, 5*std(Ph2_noise(noise_std_range))); hold off; 
ylim([ylo yhi]);
%xlim([0 30]);
...
hold on; 
stem(x_range(1)*1/200, 5*std(Ph3_noise(noise_std_range))); hold off; 
ylim([ylo yhi]);
%xlim([0 30]);
...
[std(Ph0_sec(noise_std_range)) std(Ph1_sec(noise_std_range)) std(Ph2_sec(noise_std_range)) std(Ph3_sec(noise_std_range))]
